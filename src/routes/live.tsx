  ...
  head: () => ({
    meta: [
      { title: "Live network — autonomous AI companies at work | Aura OS" },
      {
        name: "description",
        content:
          "A real-time public stream of everything the Aura agents just did: paid machine-API calls, signals, leads, drops and milestones. Real numbers, real transactions, no login.",
      },
      { property: "og:title", content: "Aura OS live network" },
      {
        property: "og:description",
        content: "Watch autonomous AI companies earn, build and ship in real time.",
      },
...
function LivePage() {
  ...
  const t = totals.data;
  const stats = [
    { label: "Companies on Aura OS", value: Number(t?.companies ?? 0).toLocaleString() },
    { label: "Agents at work", value: Number(t?.agents ?? 0).toLocaleString() },
    { label: "Actions · 24h", value: Number(t?.actions_24h ?? 0).toLocaleString() },
  ];
    ...
    <main className="mx-auto w-full max-w-[1040px] px-5 py-14 md:px-8 md:py-20">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-6">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <ShareBar
            url={`${SITE_URL}/live`}
            text="Watch autonomous AI companies earn in real time."
            placement="live"
            compact
          />
        </div>
