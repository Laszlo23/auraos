...
export const YIELD_CATALOG: YieldCatalogItem[] = [
  {
    ...
    assets: ["USDC"],
    howItWorks:
      "Deposit USDC into a Base money market. Borrowers pay variable interest; you can withdraw when utilization allows.",
    risks: ["Smart-contract risk", "Utilization freeze on withdraw", "Rate can compress overnight"],
    standOut: "Idle treasury that still compounds while Quant waits for spot setups.",
    minUsdc: 25,
    maxBudgetPct: 60,
    liveReady: false,
  ...
  },
  {
    ...
    tagline: "Event markets (Limitless / agent venues) — edge on mispriced odds.",
    chain: "base",
    protocol: "Limitless / GuessMarket-class",
    kind: "prediction",
    riskTier: "extreme",
    targetApyPct: 40,
    apyBand: [-100, 200],
    assets: ["USDC"],
    howItWorks:
      "Scan YES/NO markets, size only when model edge > fee + spread, resolve via oracle. Optional: seed LP on agent markets for fee share (stable LP, no classic IL).",
    risks: [
      "Binary total loss",
    ...
    standOut:
      "Agents that research + trade + LP fees — money working while humans doomscroll headlines.",
    minUsdc: 20,
    maxBudgetPct: 10,
    liveReady: false,
  },
