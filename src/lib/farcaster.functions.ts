...
export const getFarcasterCapabilities = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    const { neynarApiConfigured, farcasterWriteConfigured } = await import(
      "@/lib/farcaster-neynar.server"
    );
    return {
...
export const searchFarcasterCastsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { neynarApiConfigured, searchFarcasterCasts } = await import(
      "@/lib/farcaster-neynar.server"
    );
...
export const fetchFarcasterChannelFeedFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { neynarApiConfigured, fetchChannelFeed } = await import(
      "@/lib/farcaster-neynar.server"
    );
    if (!neynarApiConfigured()) throw new Error("NEYNAR_API_KEY is not set");
...
export const searchFarcasterUsersFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { neynarApiConfigured, searchFarcasterUsers } = await import(
      "@/lib/farcaster-neynar.server"
    );
    if (!neynarApiConfigured()) throw new Error("NEYNAR_API_KEY is not set");
