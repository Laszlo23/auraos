...
import type { OsCheckoutPlan } from "@/lib/os-pricing";

/** Start Stripe Checkout for Aura OS (monthly or yearly). */
export async function startFoundingSeatCheckout(
  invite?: string | null,
  plan: OsCheckoutPlan = "year",
): Promise<string> {
  const { data: sessionData } = await supabase.auth.getSession();
  const token = sessionData.session?.access_token;
  if (!token) throw new Error("Sign in first to start Aura OS.");
  ...
  const res = await fetch("/api/billing/founding-seat", {
    ...
    body: JSON.stringify({
      invite: invite?.trim().toUpperCase() || undefined,
      plan,
    }),
  });
