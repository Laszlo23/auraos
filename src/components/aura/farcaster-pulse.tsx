    ...
    <Panel label="Farcaster pulse" glow data-tour="farcaster-pulse">
      ...
      {mode === "people" ? (
        <div className="mt-5 space-y-3">
          {people == null ? (
            <p className="text-[13px] text-muted-foreground">
              Search a username to see profile cards.
            </p>
          ) : people.length === 0 ? (
        ...
        <div className="mt-5 space-y-3">
          ...
          {casts.length === 0 && !(mode === "feed" && feedQ.isLoading) ? (
            <p className="text-[13px] text-muted-foreground">
              {mode === "search"
                ? "Run a search to see casts."
                : "No casts in this channel right now."}
            </p>
          ) : (
            casts.map((c) => (
              <div key={c.hash} className="rounded-2xl border border-border/40 px-4 py-3">
