# Aura OS — grant presentation video script (~90–120s)

Use with the [Aura OS lockup](/brand/aura-logo.png) on screen. Record landscape 16:9. Speak calmly; cut B-roll of `/live`, `/grants`, `/lokal`, `/nachbar`.

**On-screen title card:** Aura OS + Aura Local · aibusiness.fun  
...
| 0:38–0:55 | “Aura Local is the same organism for neighborhood shops. Reputation after a real visit. Guest check-in. Review invites — never fake stars.” | /lokal hero · /kunden QR · review bridge |
...
## Short cut (~45s) — if a portal caps length
