...
- **Token launch (platform)** — self-hosted AURA T-0 on Uniswap v2 (Base): `AuraToken` + `AuraLpSink` + `AuraPauraRedeem`, then Hood escrow `proposeV2Market` → 72h → buy. See `contracts/aura/README.md`. Marketing countdown: `TOKEN_LAUNCH_*`. **Not** Clanker (Clanker is company tokens only).
